...
function createLp4LearningAuthority(deps={}) {
 ...
 async function canonical(resourceId,method,payload={}) {
   const hint=inferType(obj(payload).record||obj(payload).test||payload,method); const candidates=hint?[canonicalCollection(hint)]:["contentItems","studyRoadmaps","mentorLiveSessions","experienceEvents","currentAffairs"];
 ...
 async function writeCanonical(method,payload,a,meta,opts={}) {
   const p=obj(payload), record=safeClone(p.record||p.test||p), resourceId=resourceIdFrom(p); if(!resourceId) throw Object.assign(new Error("Resource id is required."),{lp4Code:"INVALID_REQUEST"}); const type=inferType(record,method); const collection=canonicalCollection(type); const current=await firestore.collection(collection).doc(resourceId).get(); const before=current.exists?obj(current.data()):{};
   ...
   const version=Math.max(Number(before.version||0),Number(record.version||0))+(method.startsWith("publish")?1:0); const doc={...before,...record,id:resourceId,resourceId,resourceType:type||before.resourceType,status:nextStatus,publishState:nextStatus,version,lp4Record:record,updatedBy:a.uid,serverUpdatedAt:serverStamp()};
   await firestore.collection(collection).doc(resourceId).set(doc,{merge:true});
   if(type==="note" && Array.isArray(record.pages)) await firestore.collection("learningTexts").doc(resourceId).set({resourceId,pages:record.pages,version,status:nextStatus,serverUpdatedAt:serverStamp()},{merge:true});
   if(type==="current-affairs") await firestore.collection("currentAffairs").doc(resourceId).set({...record,resourceId,version,status:nextStatus,serverUpdatedAt:serverStamp()},{merge:true});
 ...
 async function operation(auth,request={}) {
   ...
   let resource=null; if(resourceId && !policy.action.startsWith("ADMIN_")) { resource=await canonical(resourceId,method,payload); if(["ATTEMPT","SUBMIT","VIEW_RESULT","WATCH","DOWNLOAD","JOIN","JOIN_ATTENDANCE","PROGRESS","READ_PROGRESS","WATCH_PROGRESS","STUDY_ACTION","REMINDER"].includes(policy.action)) await requireEntitlement(a,resource); }

   const run=async()=>{
     if(method==="loadNotesAdminWorkspace") return {records:(await listCollection("contentItems")).filter(x=>inferType(x)==="note").map(x=>x.lp4Record||x)};
     if(method==="loadMockAdminWorkspace") return {tests:(await listCollection("contentItems")).filter(x=>inferType(x)==="test").map(x=>x.lp4Record||x),questionBank:await listCollection("questionBank")};
     if(method==="loadVideoAdminWorkspace") return {records:(await listCollection("contentItems")).filter(x=>["video","replay"].includes(inferType(x))).map(x=>x.lp4Record||x)};
     if(method==="loadCurrentAffairsSourceRegister") return {sources:await listCollection("currentAffairsSources"),records:(await listCollection("contentItems")).filter(x=>inferType(x)==="current-affairs").map(x=>x.lp4Record||x)};
     ...
     if(method==="duplicateMockTest") { if(!resourceId)throw Object.assign(new Error("Test id required."),{lp4Code:"INVALID_REQUEST"}); const src=await canonical(resourceId,method,payload); if(!src)throw Object.assign(new Error("Test not found."),{lp4Code:"NOT_FOUND"}); const nextId=clean(payload.newResourceId)||`${resourceId}-copy-${stableOperationId(a,method,meta,'copy').slice(0,10)}`; const record={...(src.lp4Record||src),id:nextId,resourceId:nextId,status:"Draft",publishState:"Draft",version:1}; await firestore.collection("contentItems").doc(nextId).set({...record,resourceType:"test",lp4Record:record,serverUpdatedAt:serverStamp()},{merge:false}); return {resourceId:nextId,resourceType:"test",version:1,status:"Draft"}; }
     ...
     if(method==="updateMockResultPolicy") { const ref=firestore.collection("contentItems").doc(resourceId); await ref.set({resultsPolicy:safeClone(payload.resultsPolicy||payload.policy||payload),serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId}; }
     ...
     if(method==="saveLiveReminder") { const id=`${a.uid}_${resourceId}`; await firestore.collection("liveReminders").doc(id).set({ownerUid:a.uid,sessionId:resourceId,active:true,serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId,ownerUid:a.uid,active:true}; }
     if(method==="recordRoadmapProgress"||method==="rescheduleRoadmap") { const id=`${a.uid}_${resourceId}`; await firestore.collection("studyRoadmapProgress").doc(id).set({ownerUid:a.uid,resourceId,day:Number(payload.day||payload.currentDay||1),progress:Math.max(0,Math.min(100,Number(payload.progress)||0)),rescheduleRequested:method==="rescheduleRoadmap",serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId,ownerUid:a.uid,day:Number(payload.day||payload.currentDay||1),progress:Number(payload.progress||0)}; }
     ...
     if(["uploadNoteAsset","uploadVideoAsset","processVideoAsset","saveVideoCaptions"].includes(method)) { const assetId=clean(payload.assetId||payload.attachmentId)||`${resourceId}-${stableOperationId(a,method,meta,'asset').slice(0,16)}`; await firestore.collection("protectedContentAssets").doc(assetId).set({assetId,resourceId,kind:method,storagePath:clean(payload.storagePath||payload.path),captions:safeClone(payload.captions),status:method==="processVideoAsset"?"processed":"ready",serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId,assetId,status:method==="processVideoAsset"?"processed":"ready"}; }
     ...
     if(["createLiveProviderSession","updateLiveProviderSession"].includes(method)) { await firestore.collection("mentorLiveSessions").doc(resourceId).set({...safeClone(payload.record||payload),resourceId,serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId}; }
     ...
     if(method==="createLiveReplayDraft") { const nextId=clean(payload.replayResourceId)||`${resourceId}-replay`; await firestore.collection("mentorLiveSessions").doc(nextId).set({...safeClone(payload.record),resourceId:nextId,resourceType:"replay",status:"Draft",sourceSessionId:resourceId,serverUpdatedAt:serverStamp()},{merge:true}); return {resourceId:nextId,resourceType:"replay",status:"Draft"}; }
